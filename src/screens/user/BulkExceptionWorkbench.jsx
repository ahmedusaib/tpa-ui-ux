...
// Role-appropriate SVG icons for action buttons
const IcoSearch = () => (
...
const IcoScale = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <path d="M3 7l9-4 9 4"/>
    <path d="M5 11 3 7l4-2"/><path d="M19 11l2-4-4-2"/>
    <path d="M3 14s0 3 4 3 4-3 4-3H3z"/>
    <path d="M13 14s0 3 4 3 4-3 4-3h-8z"/>
  </svg>
);
...
const CATEGORY_ACTION = {
  data:        { label: 'Search & Correct',    Icon: IcoSearch },
  eligibility: { label: 'Review Eligibility',  Icon: IcoClipboard },
  document:    { label: 'Request Replacement', Icon: IcoFileRequest },
  duplicate:   { label: 'Compare & Resolve',   Icon: IcoScale },
};
        ...
        {EXCEPTION_ROWS.map((row, i) => {
          ...
          return (
            <div key={row.row} style={{
              ...
              <span style={{ fontSize: '11px', color: T.textMuted, lineHeight: 1.3 }}>{row.errorDetail}</span>
              {isResolved ? (
                <span style={{ fontSize: '12px', fontWeight: 700, color: T.commitGreen, display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <IcoCheckCircle /> Resolved
                </span>
              ) : (
